...
import { createOrder, uploadProofOfPayment } from "@/lib/api";
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (values.paymentMethod === "gcash" && values.proofOfPayment) {
        const uploaded = await uploadProofOfPayment(values.proofOfPayment);
      ...
      const order = await createOrder({
        ...
        items: checkoutItems.map((item) => ({
          productId: item.productId, // already set to product._id by buildCartItem
